...
function creatingFloors(floor, lifts) {
   ...
   while (floor--) {
      ...
      btnUp.addEventListener('click', buttonHandler);
      btnDown.addEventListener('click', buttonHandler);
   ...
   liftsA.push({ currentFloor: 1, busy: false, translateY: 0 });
...
let queueInterval;
const requestAddedEvent = new Event("requestInvoked");
const dispatchIdleLift = new Event("dispatchLiftEvent");

function requestInvokedFnc() {
   document.dispatchEvent(requestAddedEvent);
}

function dispatchIdleLiftFnc() {
   document.dispatchEvent(dispatchIdleLift);
}

const buttonHandler = (e) => {
   const btn = e.target;
   console.log(btn.getAttribute("id"));
   console.log(parseInt(btn.getAttribute("id").match(/\d+/)));
   const floor = parseInt(btn.getAttribute("id").match(/\d+/));
   queue.add({ destinationFloor: floor, btn: btn });
...
const fcnx = (queue) => {
   ...
   let { destinationFloor, btn } = queue.shift();
   findNearestLift2(destinationFloor, btn);
}

function findNearestLift2(destinationFloor, btn) {
   let minDistance = Infinity;
   let nearestLiftId = -1;

   for (let i = 1; i < liftsA.length; i++) {
      let distance;
      if (!liftsA[i].busy) {
         distance = Math.abs(liftsA[i].currentFloor - destinationFloor);
      }
      // else {
      //   // If the lift is moving, consider its destination floor
      //   distance = Math.abs(lift.getDestinationFloor() - destinationFloor);
...
const moveLift2 = (liftIndex, btn, destinationFloor, queue) => {
   queue.remove();
   liftsA[liftIndex].busy = true;

   console.log(`Free lift available initiating lift-${liftIndex}`);

   // document.querySelector(`#${btnDirection}`).disabled = true;

   // Logic to move the Lift to the desired Floor
   // let liftCurrentFloor = allLifts[i].getAttribute("currentFloor");
   let liftCurrentFloor = liftsA[liftIndex].currentFloor;
   ...
   let freelift = document.querySelector(`#lift-${liftIndex}`);
   ...
   setTimeout(() => {
      liftsA[liftIndex].currentFloor = destinationFloor;
      let doors = doorAnimation(freelift);
      document.querySelector(`#counter-${liftIndex}`).innerHTML = liftsA[liftIndex].currentFloor;
      setTimeout(() => {
         liftsA[liftIndex].busy = false;
         doors[0].classList.remove("left-door-animation");
         doors[1].classList.remove("right-door-animation");
         dispatchIdleLiftFnc();
         btn.disabled = false;
...
const callLift = () => {
   if (queue.size() > 0) {
      let { destinationFloor, btn } = queue.front();
      console.log(queue.printQueue());
      let nearestLiftId = findNearestLift2(destinationFloor, btn);
      console.log(`${nearestLiftId}`);
      if (nearestLiftId > 0) {
         moveLift2(nearestLiftId, btn, destinationFloor, queue);
      } else {
...
document.addEventListener("requestInvoked", () => {
   callLift();
})
